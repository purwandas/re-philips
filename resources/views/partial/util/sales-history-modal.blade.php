<!-- BEGIN READ WHO MODAL POPUP -->
<div id="sales-history-modal" class="modal container fade" data-width="600" tabindex="-1">
    <div class="modal-header" style="margin-top: 30px;margin-left: 30px;margin-right: 30px;">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
        <h4 class="modal-title"><b><span id="sales-history-title"></span></b></h4>
    </div>
    <div class="modal-body" style="margin-bottom: 30px;margin-left: 30px;margin-right: 30px; word-wrap: break-word;">
        <span id="sales-history-content">
            
        </span>
    </div>
    <div class="modal-footer" style="margin-bottom: 30px;margin-left: 30px;margin-right: 30px;">
        <button type="button" data-dismiss="modal" class="btn btn-outline dark">Close</button>
    </div>
</div>
<!-- END READ WHO MODAL POPUP -->