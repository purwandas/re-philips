        ...
        <div class="portlet light bordered">
            ...
            <div class="portlet-body" style="padding: 15px;">
                ...
                <div class="row">
                    ...
                    <table class="table table-striped table-hover table-bordered" id="posmActivityTable" style="white-space: nowrap;">
                        <thead>
                            <tr>
                                <th> No. </th>
                                <th> Date </th>
                ...
                @include('partial.modal.editposmactivity-modal')
...
<script src="{{ asset('js/handler/editposmactivity-handler.js') }}" type="text/javascript"></script>
...
<script>
    ...
    var url = 'datatable/editposmactivity';
    var order = [ [0, 'desc'] ];
    ...
    var paramFilter = ['posmActivityTable', $('#posmActivityTable'), url, tableColumns, columnDefs, order];//, exportButton];

    var paramReset = [filterId, 'posmActivityTable', $('#posmActivityTable'), url, tableColumns, columnDefs, order];
    ...
    $(document).ready(function () {
        ...
        var table = $('#posmActivityTable').dataTable({
            "processing": true,
            "serverSide": true,
            "ajax": {
                url: "{{ route('datatable.editposmactivity') }}",
                type: 'POST',
            },
            "rowId": "id",
            "columns": [
        ...
        $('#posmActivityTable').on('click', 'tr td button.deleteButton', function () {
            var id = $(this).val();
    ...
    // For editing data
    $(document).on("click", ".edit-posmactivity", function () {
        resetValidation();
        ...
        var id = $(this).data('id');
        var getDataUrl = "{{ url('editposmactivity/edit/') }}";
        var postDataUrl = "{{ url('editposmactivity') }}"+"/"+id;

        // Set action url form for update
        $("#form_editposmactivity").attr("action", postDataUrl);

        // Set Patch Method
        if(!$('input[name=_method]').length){
            $("#form_editposmactivity").append("<input type='hidden' name='_method' value='PATCH'>");
        }
