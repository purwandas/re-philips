...
<div class="page-head">
    ...
    <div class="page-title">
        <h1>SOH
            <small>manage stock on hand</small>
        </h1>
    </div>
    <!-- END PAGE TITLE -->
</div>
<ul class="page-breadcrumb breadcrumb">
    ...
    <li>
        <span class="active">SOH Management</span>
    </li>
</ul>
@endsection
...
<script src="{{ asset('js/handler/editsoh-handler.js') }}" type="text/javascript"></script>
...
<script>
    ...
    var paramFilter = ['sohTable', $('#sohTable'), url, tableColumns, columnDefs, order];//, exportButton];

    var paramReset = [filterId, 'sohTable', $('#sohTable'), url, tableColumns, columnDefs, order];
    ...
    $(document).ready(function () {
        ...
        $('#sohTable').on('click', 'tr td button.deleteButton', function () {
            var id = $(this).val();
    ...
    // For editing data
    $(document).on("click", ".edit-soh", function () {
        ...
        var id = $(this).data('id');
        var getDataUrl = "{{ url('editsoh/edit/') }}";
        var postDataUrl = "{{ url('editsoh') }}"+"/"+id;

        // Set action url form for update
        $("#form_editsoh").attr("action", postDataUrl);

        // Set Patch Method
        if(!$('input[name=_method]').length){
            $("#form_editsoh").append("<input type='hidden' name='_method' value='PATCH'>");
        }
