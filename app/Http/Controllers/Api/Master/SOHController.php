...
class SOHController extends Controller
{
    ...
    public function sohWithDate(Request $request){
        ...
        // Check SOH header
        $sohHeader = SOH::where('user_id', $user->id)->where('store_id', $content['id'])->where('date', $date)->first();

            if ($sohHeader) { // If header exist (update and/or create detail)

                try {
                    DB::transaction(function () use ($content, $sohHeader, $user, $date) {
                ...
                // Check sell in(Sell Thru) header after insert
                $sohHeaderAfter = SOH::where('user_id', $user->id)->where('store_id', $content['id'])->where('date', $date)->first();
